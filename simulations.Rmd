...
alpha_range <- seq(from = 0, to = 1, by = 0.05)
beta_range <- seq(from = 0, to = 1, by = 0.05)
...
# End generation timer and log 
  ...
  if (sum(herd_tracker_a_vectors[[generation + 1]]) + sum(herd_tracker_b_vectors[[generation + 1]]) == 2 * iterations) {
    print(paste("All iterations have herded after ", generation, " generations. Ending simulation."))
    correct_herd_rate[as.character(beta), as.character(alpha)] <- sum(decision_matrix_a[, generation] == "A") / iterations
    break
  }
}
