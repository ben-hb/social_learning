...
infer_on_signal <- function(signal_matrix, decision_matrix, prior, generation){
  ...
  if (generation > 1) {
    prior <- as.numeric(prior[c(match(decision_matrix[, generation], prior)), 4])
  }
...
# Generate posterior with signal 
  ...
  if (generation == 1) {
    ...
    posterior_matrix_a[, generation] <- infer_on_signal(signal_matrix_a, decision_matrix_a, pi, generation)
    posterior_matrix_b[, generation] <- infer_on_signal(signal_matrix_b, decision_matrix_b, pi, generation)
...
# Generate posteriors from action posterior matrix with signal
  ...
  posterior_matrix_a[, generation] <- infer_on_signal(signal_matrix_a, decision_matrix_a, 
                                             action_posterior_matrices[[paste("action_posterior_matrix", 
                                                                              generation, sep = "_")]],
                                             generation)
  
  posterior_matrix_b[, generation] <- infer_on_signal(signal_matrix_b, decision_matrix_b, 
                                             action_posterior_matrices[[paste("action_posterior_matrix", 
                                                                              generation, sep = "_")]],
                                             generation)
  }
